...
function compactContextDynamicVariables(input: {
  ...
  return {
    ...
    bookable_service_names_text: truncate(input.context.bookable_service_names_text || input.context.safe_service_names_text || safeServiceNamesText(input.context), 1200),
...
export async function buildElevenLabsConversationInitiationClientData(
  ...
  try {
    ...
    console.info("ElevenLabs conversation-initiation shared demo context.", {
      ...
      bookable_service_count: context.bookable_service_names_text.split(",").map((name) => name.trim()).filter(Boolean).length,
      bookable_service_names: context.bookable_service_names_text,
